...
* Adjustment for series break follows: 
...
label values realinc  gsp001x;
label values conrinc  gsp002x;
label values dateintv gsp003x;
label values cohort   gsp004x;
label values birthmo  gsp005x;
label values ballot   gsp006x;
label values sample   gsp007x;
label values spanint  gsp008x;
label values form     gsp009x;
label values health   gsp010x;
label values marital  gsp011x;
label values age      gsp012x;
label values degree   gsp013x;
label values sex      gsp014x;
label values hompop   gsp015x;
label values rincome  gsp016x;
...
* Make adjustments for series breaks (follows Stevenson, B., & Wolfers, J. (2008). Economic Growth and Subjective Well-Being: Reassessing the Easterlin Paradox. Brookings Papers on Economic Activity, 1–87.)
gen MARITAL=marital
...
drop if year>2006 
drop _Iyear_2008 _Iyear_2010 _Iyear_2012 _Iyear_2014 _Iyear_2016
drop if marital==9 | degree==9 | degree==8 | realinc==0
drop if age ==98 | age==99

gen fem = (sex==2)
gen mrd = (marital==1)
gen dgr = (degree==3 | degree==4)
gen age2 = age^2 

* generate income per member
gen inc_pm = realinc/hompop
...
keep hppy inc_pm_n age age2 dgr fem  mrd _Iyear* wt_correction year 
order hppy inc_pm_n age age2 dgr fem  mrd  _Iyear* wt_correction year 
