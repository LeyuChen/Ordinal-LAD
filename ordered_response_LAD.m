...
% mipfocus : change here for using different Gurobi solution algorithms
...
function [bhat,val,gap,rtime,ncount] = ordered_response_LAD(y,x,w,J,beta0,N,T,bnd,tau,guess)
...
y_diff = ( abs( (repmat(y,1,J-1)-(1:(J-1))) .*(tau - ( (repmat(y,1,J-1)-(1:(J-1))) <0) )) - ...
           abs((repmat(y,1,J-1)-(2:J))    .*(tau - ((repmat(y,1,J-1)-(2:J))<0))) ).*w;
