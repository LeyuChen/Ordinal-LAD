...
function [CI bhat_boot boot_m_py] = CI_ordinal_LAD(y,x,w,J,beta0,bnd,bhat,boot_m,boot_rep,alpha,N,T,sampling,tau,guess)
...
ind = [];
boot_m_py = [];
ind_temp = find(sum(x(:,mm-8:mm),2)==0); 
boot_m_py = ceil(sum(sum(x(:,mm-8:mm),2)==0)/n*boot_m);

ind = ind_temp(unidrnd(size(ind_temp,1),boot_m_py,boot_rep)); % for omitted 2000
for ii = mm-8:mm % for 2002 to 2018
...
[bhat_boot(:,i),~,gap(i),rtime(i),ncount(i)]  = ordered_response_LAD(y(ind(:,i)),x(ind(:,i),:),w(ind(:,i)),J,beta0,N,T,bnd,tau,guess);
end
